defmodule TemplatesTest do
  ...
  test "it can copy image files as is" do
    source = "test/data/image.png"
    dest = "dest.png"
    MT.copy_tree_with_expansions(source, dest, assigns: [])
    assert File.read!(source) == File.read!(dest)
    File.rm_rf!(dest)
  end

  test "handles escape hatches for assigns in templates" do
    source  = "_in.html.eex"
    dest    = "_out.html.eex"
    File.write!(source, "first: <%%= @one %>\nsecond: <%= @two %>")
    MT.copy_and_expand(source, dest, assigns: [two: 2])
    result = File.read!(dest)
    File.rm_rf!(source)
    File.rm_rf!(dest)
